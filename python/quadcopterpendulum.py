...
class QuadcopterPendulum:
    def __init__(self, mass=2.5, length=.5, guess=1, withDisplay=False):
        ...
        self.qup = np.matrix([2., 2., 2., np.pi/2, np.pi/2, np.pi/5, np.pi/5 ]).T
        self.qlow = -self.qup
        self.vup = np.matrix([2., 2., 2., 2., 2., 1., 1.]).T
        self.vlow = -self.vup
        # self.xup = np.vstack([self.qup, self.vup])
        # self.xlow = np.vstack([self.qlow, self.vlow])

        # These bounds are used for learning.
        self.qmax = np.matrix([5., 5., 5., np.pi/2, np.pi/2, np.pi/3, np.pi/3]).T
        self.vmax = np.matrix([5]*7).T
...
if __name__ == '__main__':
    env = QuadcopterPendulum(withDisplay=True)
